...
const getFormData = () => ({
  ...
  roundingAmount: Number(getField('roundingAmount').value || 0),
...
const calculateTotals = (data) => {
  ...
  const rawTotal = taxable + igv + mobilitySubtotal;
  const rounding = Math.min(Math.max(data.roundingAmount || 0, 0), rawTotal);
  return {
    subtotal,
    discount,
    igv,
    mobilitySubtotal,
    total: rawTotal - rounding,
  };
};
